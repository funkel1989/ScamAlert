...
public sealed class PortalContactService(ScamAlertDbContext dbContext) : IPortalContactService
{
    ...
    {
        var e164 = ValidateContact(request.FullName, request.PhoneNumber, request.EscalationOrder);
    ...
    private static string ValidateContact(string fullName, string phoneNumber, int escalationOrder)
    {
        ...
        if (!UsPhoneNumber.TryNormalize(phoneNumber, out var e164, out var phoneError))
        {
        ...
        if (e164.Length > 32)
        {
            throw new ArgumentException(UsPhoneNumber.InvalidMessage);
        }
